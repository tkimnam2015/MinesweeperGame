...
namespace TROCHOIDOMIN
{
    public partial class Form1 : Form
    {
        ...
        public void StartGame(int nBoom, int nCot, int nHang)
        {
            ...
            cell = new CELL[nHang, nCot]; //game với nHàng và n cột
            khoitao(nCot, nHang);
            //VẼ Ô
            ...
            { //cho dòng while chạy đến khi đặt đc hết số boom truyền vào
                int h = rd.Next(1, nHang);
                int c = rd.Next(1, nCot);
                if (cell[h, c].boom != -1) //nếu ô [h,c] được chọn đặt boom chưa được đặt
                {
                    cell[h, c].boom = -1; // cho ô này là boom
        ...
        public void paintCELL(int nBoom, int nCot, int nHang) {
            ...
            panel2.Size = new System.Drawing.Size(size * nCot, size * nHang+5);
            this.Size = new System.Drawing.Size(size * nCot + 7, size * nHang + 86);
            ...
            Color a = System.Drawing.Color.PaleGreen;
            Color b = System.Drawing.Color.LightGreen;
            for (int i = 0; i < nHang; i++)
            {
                for (int j = 0; j < nCot; j++)
                {
            ...
            for (int i = 0; i < nHang; i++)
            {
                for (int j = 0; j < nCot; j++)
                {
                    if (cell[i, j].boom == -1)
                    {
                        ...
                        cell[i, j].bt.BackColor = System.Drawing.Color.LemonChiffon;
                        cell[i, j].open = true;
                    }
                    else
                    {
